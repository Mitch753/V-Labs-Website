// Note: Modified Gulp file for SCSS admin styles
// Note: Modified Gulp file for conditional Savvii cache flush
...
const adminCssInput = './scss/admin.scss';
...
function images() {
	return src(imagesInput).pipe(dest(imagesOutput));
}

function fonts() {
	return src(fontsInput).pipe(dest(fontsOutput));
}

function flushcache() {
	url = process.cwd();
	url = url.replace('source', '') + './.vscode/sftp.json';
	...
	fs.readFile(url, 'utf-8', function (err, data) {
		...
		if (remotePath.includes('wordpress/current')) {
			remote = data.remotePath.replace('/wordpress/current', '');
			cacheurl = 'https://' + host + '' + remote + '__flushcache.php';

			if (cacheurl) {
				return request(cacheurl, function (error, response) {
					if (!error && response.statusCode === 200) {
						console.log('Site cache flushed: ' + cacheurl);
					} else {
						console.log(
							'ERROR: Site cache not flushed: ' + cacheurl
						);
					}
				});
			}
...
exports['a-watch'] = function () {
	...
	watch('./scss/**/*.*', function (file) {
		if (file.event === 'unlink') {
			fs.unlink(cssOutput + '/style.min.css', () =>
